...
from lsl_definitions.utils import Deprecated, remove_nones, splice_str

_EMBEDDED_BUILTINS_SOURCE_COMMENT = "/* GENERATED SECONDLIFE BUILTIN SOURCE */"


def _write_builtin_globals(defs: io.StringIO, slua_definitions: SLuaDefinitions) -> None:
    for const in slua_definitions.builtin_constants:
        if const.name in {"nil", "true", "false"}:
            continue
        defs.write("declare ")
    ...
    for func in slua_definitions.builtin_functions:
        defs.write("declare ")
        func.write_luau_global_def(defs, indent=0)
    defs.write("\n")


def _write_slua_library(defs: io.StringIO, slua_definitions: SLuaDefinitions) -> None:
...
def _gen_slua_definition_source(
    slua_definitions: SLuaDefinitions, *, include_builtin_globals: bool
) -> str:
    defs = io.StringIO()
    if include_builtin_globals:
        defs.write(
        ...
        _write_builtin_globals(defs, slua_definitions)

    _write_slua_library(defs, slua_definitions)
...
@register("slua_embedded_builtin_definitions")
def gen_slua_embedded_builtin_definitions(
    definitions: LSLDefinitions, slua_definitions: SLuaDefinitions, template_path: str
) -> str:
    """Generate secondlife/slua's EmbeddedBuiltinDefinitions.cpp"""
    with open(template_path, "r", encoding="utf8") as f:
        template = f.read()

    builtin_source = _gen_slua_definition_source(slua_definitions, include_builtin_globals=True)
    return splice_str(template, _EMBEDDED_BUILTINS_SOURCE_COMMENT, builtin_source.rstrip())
